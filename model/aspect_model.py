...
class ASPECTModel(BaseModel):
    ...
    # conv = tf.squeeze(conv) # ( (BATCH_SIZE*WORDS), WINDOW_LEN-FILTER_SIZE + 1, NUMBER_OF_FEATURE_MAPS)
                ...
                pooled = tf.nn.max_pool(conv,ksize=[1, 2 , 1, 1],strides=[1, 1, 1, 1],padding='VALID',data_format = 'NHWC',name="pool")
                pooled = tf.squeeze(pooled) # ( (BATCH_SIZE*WORDS), NUMBER_OF_FEATURE_MAPS)
                self.output = tf.reshape(pooled, (-1, tf.shape(self.word_ids)[1], (self.config.WINDOW_LEN-filter_size) * self.config.NUMBER_OF_FEATURE_MAPS[i]))
